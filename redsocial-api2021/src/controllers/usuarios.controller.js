...
usuarios.getusuario = async (req, res) => {
  ...
  const response = await conexion.query(
    `select *from usuario where nombres_usuario ilike '%${busqueda}%' or apellidos_usuario ilike  '%${busqueda}%' or email_usuario ilike  '%${busqueda}%' or nom_usuario ilike  '%${busqueda}%'`
  );
  res.status(200).json(response.rows);
};
usuarios.registro = async (req, res) => {
  ...
  if (emailExistente) {
    ...
    if (usuarioExistente) {
      return res
        .status(400)
        .send("El usuario ingresado esta actualmente en uso.");
    } else {
      let query = `INSERT INTO public.usuario(nombres_usuario, apellidos_usuario, fecha_nac_usuario, email_usuario, contrasena_usuario, presentacion_usuario, telefono_usuario, id_genero,nom_usuario,id_provincia) VALUES ('${nombres_user}','${apellidos_user}','${fecha_nac}','${email_user}','${hash}','${presentacion}','${telefono}','${id_genero}','${nom_usuario}','${id_provincia}')`;
      await conexion.query(query);
      const result = await conexion.query(
        "SELECT  MAX(id_usuario) FROM usuario LIMIT 1"
      );
...
usuarios.confirmContrasena = async (req, res) => {
  ...
  if (!bcrypt.compareSync(contrasena_usuario, user.rows[0].contrasena_usuario))
    return res.status(401).json({ message: "Password erroneo" });
  ...
  passport.authenticate("local-login", (err, usuario, info) => {
    try {
      ...
      req.logIn(usuario, (err) => {
        ...
        req.session.id_provincia = usuario.id_provincia;
        user = req.user;
        res.json({ user, token });
      });
    } catch (error) {
...
usuarios.authUsuario = async (req, res) => {
  ...
  const response = await conexion.query(
    "select usuario.id_usuario, nombres_usuario,apellidos_usuario,fecha_nac_usuario,email_usuario,presentacion_usuario,telefono_usuario,nom_usuario,imagen_usuario,usuario.id_genero,nombre_genero,id_provincia from usuario INNER JOIN genero ON usuario.id_genero=genero.id_genero WHERE id_usuario = $1",
    [id]
  );
  res.status(200).json(response.rows);
};
